...
def play(strategy0, strategy1, update,
    ...
    while True:
        if who == 0:
            ...
            score0 = update(roll, score0, score1, dice)
            if score0 >= goal:
                break
        if who == 1:
            roll = strategy1(score1, score0)
            score1 = update(roll, score1, score0, dice)
            if score1 >= goal:
                break
        who = 1 - who
    # END PROBLEM 5
...
def make_averaged(original_function, times_called=1000):
    ...
    # BEGIN PROBLEM 8
    def wrapper(*args):
        sum_rolls = sum(original_function(*args) for _ in range(times_called))
        return sum_rolls / times_called
    return wrapper
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    for roll in range(1, 11):
        score = make_averaged(roll_dice, times_called)(roll, dice)
        if score > max_scroe:
            max_roll = roll
            max_scroe = score
    return max_roll
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    new_scroe = sus_points(score + boar_brawl(score, opponent_score))
    if new_scroe - score >= threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
